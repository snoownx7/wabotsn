	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = ｀ Adeus @${num.split('@')[0]}trouxa do carai 🤡👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			switch(command) {
				...
				/*case 'loli':
					...
					break
				case 'nsfwloli':
					if (!isNsfw) return reply('❌ *DESLIGADO* ❌')
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que você está procurando Zé ruela 🤡 ')
					...
					break
				case 'tiktok':
					if (args.length < 1) return reply('Onde está o url fdp?')
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Onde está o nome dele?', text, {quoted: mek})
						...
						reply('Nome de usuário não é válido')
					}
					break
				case 'nulis':
				case 'tulis':
				...
				case 'clearall':
					...
					reply('Sucesso ao limpar o chat :)')
					break
				case 'bc':
					if (!isOwner) return reply('Quem é Você?')
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('❌ Falha ao converter a figurinha em imagem. tente novamente! ❌')
					...
					break
				case 'simi':
					if (args.length < 1) return reply('Onde tá o texto? ')
					teks = body.slice(5)
